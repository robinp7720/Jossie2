...
async fn persist_final_assistant_response(
    ...
    let openai_optimizations = state.agent.openai_optimizations;
...
async fn maybe_summarize_context(state: &AppState, conv_id: Uuid, messages: &mut Vec<Message>) {
    ...
    let keep_recent = state.agent.context_keep_recent_dialogue_messages;
    ...
    if total_chars < state.agent.max_context_chars || messages.len() <= keep_recent {
        return;
    }
