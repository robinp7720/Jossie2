...
impl AppState {
    /// Persist and broadcast an event in one ordered operation.
    ///
    /// Returning the derived work events lets a request-scoped stream mirror the
    /// same sequence without re-running persistence.
    pub async fn publish_event(&self, event: ServerEvent) -> Vec<ServerEvent> {
        ...
        for work_event in &work_events {
            let _ = self.event_tx.send(work_event.clone());
        }
        work_events
    }
